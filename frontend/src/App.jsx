...
function App() {
  ...
  const fetchPartners = useCallback(async () => {
    ...
    try {
      ...
      const partnersWithFiles = data.map((partner) => {
        const partnerId = partner.key || uuidv4();
        return {
          id: partnerId,
          name: partner.partner,
          logo: partner.icon ? (partner.icon.startsWith('/') ? partner.icon : '/' + partner.icon) : undefined,
          detection_settings: partner.detection,
          password: partner.password,
          key: partner.key,
          files: (partner.files || []).map((file, idx) => {
            ...
            return {
              ...
              state: file.anonymized ? 'Anonymized' : 'De-anonymized',
  ...
  const handleFileUpload = async (filesToUpload) => {
    ...
    try {
      ...
      console.log("====== Checking analysis result=========")
  ...
  const handleProceedAnonymization = async (updatedReviewItems) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/process`, {
        ...
        body: JSON.stringify(updatedReviewItems),
      });
  ...
  const handleToggleFileAnonymization = async (partner, filename, shouldAnonymize) => {
    const endpoint = shouldAnonymize ? "deanonymize" : "anonymize";
    try {
      ...
      // Refresh the partners list using the main fetch function
      await fetchPartners();
    } catch (error) {
  ...
  const handleViewAuditLog = (file) => {
    setAuditLogData(file);
    setCurrentFileBeingReviewed(file);
